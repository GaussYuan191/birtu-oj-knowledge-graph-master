            ...
            xhr.onreadystatechange = function() {//Call a function when the state changes.
                if(xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200) {//xhr.readyState == 4等价于XMLHttpRequest.DONE
                    ...
                    if(jsoncontent['code'] == 200){
                        ...
                        jsondiv.innerHTML = message;
                        alert(message);
                    }
                    // jsondiv.innerHTML = JSON.stringify(jsoncontent);/*将json对象转成字符串 并写入 中第一个元素的name属性对应的值写入jsondiv中*/
                    // alert(JSON.stringify(jsoncontent));
                    contentdiv.appendChild(jsondiv);/*将jsondiv放入DOM中，作为contentdiv的子元素*/



                       // alert(JSON.stringify(obj));    //将json对象转化为字符串
